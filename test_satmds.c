...
/* Saturnin MUL on 64-bit: {a,b,c,d} → {b,c,d,a^b} */
static inline void MUL64(uint64_t *t0,uint64_t *t1,uint64_t *t2,uint64_t *t3){
    uint64_t tmp=*t0; *t0=*t1; *t1=*t2; *t2=*t3; *t3=tmp ^ *t0;
}

/* Exact Saturnin MDS on 4×64-bit lanes */
static inline void saturnin_mds(uint64_t *h1,uint64_t *h2,uint64_t *h3,uint64_t *h4){
    /* C ^= D */  *h3 ^= *h4;
    /* A ^= B */  *h1 ^= *h2;
    MUL64(h2,h2,h2,h2);  /* B = MUL(B) — operate on copies? No, Saturnin uses groups of 4. */
    /* Actually the MDS operates on 4-tuples A=(x0,x1,x2,x3), B=(x4,x5,x6,x7), etc. */
    /* For PlumHash, each lane is a single 64-bit value, not a 4-tuple. */
    /* We need to adapt: treat the 4 lanes as a 4-tuple, apply MUL pattern differently. */
}

/* Correct adaptation: Saturnin MDS on a 4-element vector */
/* In Saturnin, MDS takes 16 values (4 groups of 4) and mixes them. */
/* For 4 values, we can use a 4×4 MDS matrix directly. */

/* Simple 4×4 MDS over 64-bit using only XOR + rotate (not GF multiply) */
static inline void mds4(uint64_t *a,uint64_t *b,uint64_t *c,uint64_t *d){
    uint64_t ta=*a,tb=*b,tc=*c,td=*d;
    /* MDS matrix: each output is sum of all inputs with different rotations */
...
int main(void){
    printf("=== MDS Lane Compression ===\n\n");
    struct{const char*n;hf f;} hs[]={{"Base",f_base},{"MDS4",f_mds2}}; int nh=2;
    ...
    for(int hi=0;hi<nh;hi++){printf("%-6s",hs[hi].n);
        for(int si=0;si<2;si++){size_t sz=si?256:64;uint8_t*b=malloc(sz);for(size_t i=0;i<sz;i++)b[i]=i*0x9D+0x37;
