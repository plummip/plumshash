...
int main(void){
    /* Safe-residue shifts from PRIEMFORMULE {0,1,3,4,6,7} → {17,23,29,37,43,53} */
    int shifts[]={17,23,29,37,43,53};
    int ns=6;
    int best_chi2=999,best_s1=0,best_s2=0,best_s3=0,best_s4=0;double best_ava=0;

    printf("Safe-residue shift scan (6^4=%d combos, full chi2:256K, full ava:32x8)\n\n",ns*ns*ns*ns);
    ...
    for(int a=0;a<ns;a++)for(int b=0;b<ns;b++)for(int c=0;c<ns;c++)for(int d=0;d<ns;d++){
        ...
        /* Full chi2: 256K samples */
        int bins[256]={0};for(int i=0;i<256000;i++)bins[h(&i,4,i,s1,s2,s3,s4)&255]++;
        double ex=1000.0,chi2=0;for(int i=0;i<256;i++){double d=bins[i]-ex;chi2+=d*d/ex;}
        if(chi2>300)continue; /* quick reject */
        /* Full avalanche: 32 bytes × 8 bits */
    ...
    printf("\nTested %d  BEST: {%d,%d,%d,%d} chi2=%d ava=%.1f%%\n",tested,best_s1,best_s2,best_s3,best_s4,best_chi2,best_ava);
    printf("Time: %lds\n",time(NULL)-start);
    printf("Original {29,31,37,41} chi2 test...\n");
    int bins[256]={0};for(int i=0;i<256000;i++)bins[h(&i,4,i,29,31,37,41)&255]++;
    double ex=1000.0,chi2=0;for(int i=0;i<256;i++){double d=bins[i]-ex;chi2+=d*d/ex;}
    printf("Original chi2=%.1f\n",chi2);
    return 0;
}
