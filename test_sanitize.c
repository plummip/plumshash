/*
 * test_sanitize.c — correctness/edge sanity for plumshash under
 ...
 *
 * Compile: clang -O1 -g -fsanitize=address,undefined -o test_sanitize test_sanitize.c
 */
...
int main(void) {
    ...
    for (size_t len = 0; len <= 4096; len++) {
        ...
        uint64_t h0 = plumshash(buf, len, seeds[0]);
        uint64_t h1 = plumshash(buf, len, seeds[1]);
        uint64_t h2 = plumshash(buf, len, seeds[2]);
        if (h0 != plumshash(buf, len, seeds[0])) { printf("NONDETERMINISTIC len=%zu\n", len); fails++; }
        ...
        if (len > 0 && h0 == plumshash(buf, len - 1, seeds[0])) {
    ...
    for (int li = 0; li < 2; li++) {
        size_t len = lens[li];
        uint64_t base = plumshash(buf, len, 42);
        long total = 0; int worst = 0;
        for (int b = 0; b < 8; b++) {
            for (int bit = 0; bit < 8; bit++) {
                buf[b] ^= (uint8_t)(1u << bit);
                uint64_t h = plumshash(buf, len, 42);
                buf[b] ^= (uint8_t)(1u << bit);
    ...
    {
        size_t len = 64;
        uint64_t base = plumshash(buf, len, 7);
        long total = 0;
        for (size_t b = 0; b < len; b++) {
            buf[b] ^= 0x5A;
            uint64_t h = plumshash(buf, len, 7);
            buf[b] ^= 0x5A;
    ...
    {
        uint64_t a = plumshash(buf, 256, 0);
        uint64_t b = plumshash(buf, 256, 1);
