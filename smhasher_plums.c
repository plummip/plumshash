...
static void test_sanity(void) {
    ...
    /* empty key with seed 0 is non-zero — the tiny path mixes PH_PHI+seed
     * with an XOR secret before the finalizer (deliberate: avoids the
     * all-zero fixed point on empty input) */
    TEST("empty_seed0_nonzero",
        plumshash("", 0, 0) != 0);
