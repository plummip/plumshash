...
uint64_t grid(const void*b,size_t l,uint64_t s){const uint8_t*p=b,*e=p+l;uint64_t ba=s^(l*VP),h1=ba*VP,h2=ba*VM1,h3=ba*VM2,h4=ba*VM3;
    while(p+32<=e){
        ...
        /* Row mix: h1↔h2, h3↔h4 */
        h1=rot(h1+h2,11);h2=rot(h2+h1,17);
        h3=rot(h3+h4,23);h4=rot(h4+h3,57);
        /* Column mix: h1↔h3, h2↔h4 */
        h1=rot(h1+h3,13);h3=rot(h3+h1,19);
        h2=rot(h2+h4,31);h4=rot(h4+h2,43);
        /* Diagonal mix: h1↔h4, h2↔h3 */
        h1=rot(h1+h4,29);h4=rot(h4+h1,37);
        h2=rot(h2+h3,41);h3=rot(h3+h2,53);
    }
...
/* Grid-lite: rows only (2x mixing instead of 3x) */
uint64_t grid2(const void*b,size_t l,uint64_t s){const uint8_t*p=b,*e=p+l;uint64_t ba=s^(l*VP),h1=ba*VP,h2=ba*VM1,h3=ba*VM2,h4=ba*VM3;
    while(p+32<=e){
        uint64_t v1=*(const uint64_t*)p;p+=8;uint64_t v2=*(const uint64_t*)p;p+=8;
        uint64_t v3=*(const uint64_t*)p;p+=8;uint64_t v4=*(const uint64_t*)p;p+=8;
        h1^=v1;h2^=v2;h3^=v3;h4^=v4;
...
int main(void){printf("=== Ring vs Grid Mixing ===\n\n");
    uint8_t wb[256];volatile uint64_t ws=0;for(int i=0;i<500;i++){ws^=ring(wb,256,i);ws^=grid(wb,256,i);ws^=grid2(wb,256,i);}
    struct{const char*n;hf f;}hs[]={{"Ring",ring},{"Grid3x",grid},{"Grid2x",grid2}};int nh=3;
