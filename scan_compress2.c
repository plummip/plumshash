...
uint64_t hx(const void*b,size_t l,uint64_t s,int r1,int r2){
    ...
    if(r1)h1^=rot(h2,r1);else h1^=h2;
    if(r2)h3^=rot(h4,r2);else h3^=h4;
    h1^=h3;
...
int main(void){
    ...
    int ns=26;
    printf("Lane compression scan (r1,r2 rotations):\n\n");
    double best_ava=0;int best_chi2=999,br1=0,br2=0;
    for(int a=0;a<ns;a++){int r1=shifts[a];
    for(int b=0;b<ns;b++){int r2=shifts[b];
    if(r1==r2)continue;
    uint8_t buf[256];for(int i=0;i<256;i++)buf[i]=i*0x9D+0x37;uint64_t base=hx(buf,256,0,r1,r2);double lo=100;int fail=0;
    for(int by=0;by<32&&!fail;by++)for(int bi=0;bi<8&&!fail;bi++){buf[by]^=(1u<<bi);uint64_t hv=hx(buf,256,0,r1,r2);buf[by]^=(1u<<bi);double p=pop(base^hv)/64.0*100;if(p<lo)lo=p;if(p<25)fail=1;}
    if(fail)continue;
    int bins[256]={0};for(int i=0;i<64000;i++)bins[hx(&i,4,i,r1,r2)&255]++;double ex2=250.0,chi2v=0;for(int i=0;i<256;i++){double d=bins[i]-ex2;chi2v+=d*d/ex2;}
    if(lo>best_ava||(lo==best_ava&&chi2v<best_chi2)){best_ava=lo;best_chi2=(int)chi2v;br1=r1;br2=r2;}
    if(lo>=39&&chi2v<240)printf("r1=%2d r2=%2d ava=%.1f%% chi2=%.0f\n",r1,r2,lo,chi2v);
    }}
    /* Also test baseline (0,0) */
    {uint8_t buf[256];for(int i=0;i<256;i++)buf[i]=i*0x9D+0x37;uint64_t base=hx(buf,256,0,0,0);double lo=100;
    for(int by=0;by<32;by++)for(int bi=0;bi<8;bi++){buf[by]^=(1u<<bi);uint64_t hv=hx(buf,256,0,0,0);buf[by]^=(1u<<bi);double p=pop(base^hv)/64.0*100;if(p<lo)lo=p;}
    int bins[256]={0};for(int i=0;i<64000;i++)bins[hx(&i,4,i,0,0)&255]++;double ex2=250.0,chi2v=0;for(int i=0;i<256;i++){double d=bins[i]-ex2;chi2v+=d*d/ex2;}
    printf("\nBaseline (r1=0,r2=0): ava=%.1f%% chi2=%.0f\n",lo,chi2v);
    printf("BEST: r1=%d r2=%d ava=%.1f%% chi2=%d\n",br1,br2,best_ava,best_chi2);}
    return 0;
}
