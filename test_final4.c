...
/* VortexHash with {29,17,53,43} finalizer shifts */
...
static uint64_t v_fast(const uint8_t*p,size_t l,uint64_t s){
    ...
    h1^=h2;h3^=h4;h1^=h3;
    h1^=h1>>29;h1*=VM1;h1^=h1>>17;h1*=VM2;h1^=h1>>53;h1*=VP;h1^=h1>>43;
    return h1;
}
...
uint64_t v4hash(const void*b,size_t l,uint64_t s){return l>=256?v_fast((const uint8_t*)b,l,s):v_safe((const uint8_t*)b,l,s);}
...
int main(void){
    printf("=== V4 {29,17,53,43} vs wyhash64 ===\n\n");
    struct{const char*n;hf f;}hs[]={{"VortexV4",v4hash},{"wyhash64",wyhash64}};int nh=2;
    uint8_t wb[1024];volatile uint64_t ws=0;for(int i=0;i<2000;i++){ws^=v4hash(wb,256,i);ws^=wyhash64(wb,256,i);}
    ...
    printf("\nSpeed (GB/s): %8s","");size_t szs[]={64,256,1024,65536};for(int si=0;si<4;si++)printf(" %8zu",szs[si]);printf("\n");
