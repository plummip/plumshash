...
static inline uint64_t pl_read64(const uint8_t *p) { uint64_t v; memcpy(&v, p, sizeof(v)); return v; }
...
/* ── NEW: parallel lane update + extra cross-mix + extended finalizer ── */
static uint64_t plum_fast_new(const uint8_t *p, size_t len, uint64_t seed) {
    const uint8_t *e = p + len;
    uint64_t ba = seed ^ (len * PL_PHI);
    uint64_t h1 = ba * PL_PHI, h2 = ba * PL_M1, h3 = ba * PL_M2, h4 = ba * PL_M3;
    while (p + 32 <= e) {
        uint64_t v1 = pl_read64(p); p += 8;
        uint64_t v2 = pl_read64(p); p += 8;
        uint64_t v3 = pl_read64(p); p += 8;
        uint64_t v4 = pl_read64(p); p += 8;
        uint64_t t1 = pl_rot((h1 ^ v1) + h2, 11);
        uint64_t t2 = pl_rot((h2 ^ v2) + h3, 17);
        uint64_t t3 = pl_rot((h3 ^ v3) + h4, 23);
        uint64_t t4 = pl_rot((h4 ^ v4) + h1, 57);
        h1 = t1; h2 = t2; h3 = t3; h4 = t4;
    }
    while (p + 8 <= e) { uint64_t v = pl_read64(p); p += 8; h1 ^= v; h1 = pl_rot(h1 + h2, 11); }
    ...
    switch (e - p) {
        ...
        case 1: t ^= p[0]; t *= PL_PHI; h1 ^= t; h1 = pl_rot(h1 + h2, 11);
    }
    h1 ^= h2; h3 ^= h4; h1 ^= h3;
    h1 ^= h1 >> 29; h1 *= PL_M1; h1 ^= h1 >> 31; h1 *= PL_M2;
    h1 ^= h1 >> 37; h1 *= PL_PHI; h1 ^= h1 >> 41;
    h1 ^= h1 >> 43; h1 *= PL_M3; h1 ^= h1 >> 47;
    return h1;
}
...
static uint64_t plum_safe_new(const uint8_t *p, size_t len, uint64_t seed) {
    ...
    int has_blocks = 0;
    while (p + 32 <= e) {
        uint64_t v1 = pl_read64(p); p += 8; uint64_t v2 = pl_read64(p); p += 8;
        uint64_t v3 = pl_read64(p); p += 8; uint64_t v4 = pl_read64(p); p += 8;
        uint64_t t1 = pl_rot((h1 ^ v1) + h2, 11); uint64_t t2 = pl_rot((h2 ^ v2) + h3, 17);
        uint64_t t3 = pl_rot((h3 ^ v3) + h4, 23); uint64_t t4 = pl_rot((h4 ^ v4) + h1, 57);
        h1 = t1; h2 = t2; h3 = t3; h4 = t4;
        acc ^= v1 ^ v2 ^ v3 ^ v4; acc = pl_rot(acc, 31); acc *= PL_PHI;
        has_blocks = 1;
    }
    while (p + 8 <= e) { uint64_t v = pl_read64(p); p += 8; h1 ^= v; h1 = pl_rot(h1 + h2, 11); acc ^= v; acc = pl_rot(acc, 31); acc *= PL_PHI; has_blocks = 1; }
    h2 ^= h1; h2 = pl_rot(h2, 33); h1 ^= h2;
    uint64_t t = 0;
    switch (e - p) {
        case 7: t ^= (uint64_t)p[6] << 48; case 6: t ^= (uint64_t)p[5] << 40;
        ...
        case 3: t ^= (uint64_t)p[2] << 16; case 2: t ^= (uint64_t)p[1] << 8;
        case 1: t ^= p[0]; t *= PL_PHI; h1 ^= t; h1 = pl_rot(h1 + h2, 11);
        if (has_blocks) { acc ^= t; acc = pl_rot(acc, 31); acc *= PL_PHI; }
    }
...
uint64_t plumhash_new(const void *buf, size_t len, uint64_t seed) {
    return len >= 256 ? plum_fast_new((const uint8_t *)buf, len, seed) : plum_safe_new((const uint8_t *)buf, len, seed);
}
...
int main(void) {
    printf("=== NEW vs ORIGINAL PlumHash ===\n\n");
    struct{const char*n;hf f;} hs[]={{"ORIGINAL",plumhash},{"NEW",plumhash_new}};
    int nh=2;
    ...
    for(int hi=0;hi<nh;hi++) {
        ...
        { size_t sz=64; uint8_t*b=malloc(sz); for(size_t i=0;i<sz;i++) b[i]=i*0x9D+0x37;
          volatile uint64_t s=0; clock_t st=clock(); for(int i=0;i<20000000/sz;i++) s^=fn(b,sz,i); clock_t en=clock();
          spd64=sz*(20000000/sz)/((double)(en-st)/CLOCKS_PER_SEC)/1e9; free(b); }
        { size_t sz=1024; uint8_t*b=malloc(sz); for(size_t i=0;i<sz;i++) b[i]=i*0x9D+0x37;
          volatile uint64_t s=0; clock_t st=clock(); for(int i=0;i<20000000/sz;i++) s^=fn(b,sz,i); clock_t en=clock();
          spd1k=sz*(20000000/sz)/((double)(en-st)/CLOCKS_PER_SEC)/1e9; free(b); }
