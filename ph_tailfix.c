...
/* New safe: only use accumulator if we processed at least 1 full block */
uint64_t safe_new(const void*b,size_t l,uint64_t s){const uint8_t*p=b,*e=p+l;uint64_t ba=s^(l*PHI),h1=ba*PHI,h2=ba*M1,h3=ba*M2,h4=ba*M3,acc=ba^M2;int has_blocks=0;
    while(p+32<=e){uint64_t v1=*(const uint64_t*)p;p+=8;uint64_t v2=*(const uint64_t*)p;p+=8;uint64_t v3=*(const uint64_t*)p;p+=8;uint64_t v4=*(const uint64_t*)p;p+=8;
        h1^=v1;h1=rot(h1+h2,11);h2^=v2;h2=rot(h2+h3,17);h3^=v3;h3=rot(h3+h4,23);h4^=v4;h4=rot(h4+h1,57);acc^=v1^v2^v3^v4;acc=rot(acc,31);acc*=PHI;has_blocks=1;}
    while(p+8<=e){uint64_t v=*(const uint64_t*)p;p+=8;h1^=v;h1=rot(h1+h2,11);acc^=v;acc=rot(acc,31);acc*=PHI;has_blocks=1;}
    uint64_t t=0;switch(e-p){case 7:t^=(uint64_t)p[6]<<48;case 6:t^=(uint64_t)p[5]<<40;case 5:t^=(uint64_t)p[4]<<32;case 4:t^=(uint64_t)p[3]<<24;case 3:t^=(uint64_t)p[2]<<16;case 2:t^=(uint64_t)p[1]<<8;case 1:t^=p[0];}if(t){h1^=t;h1=rot(h1+h2,11);if(has_blocks){acc^=t;acc=rot(acc,31);acc*=PHI;}}
    h1^=h2;h3^=h4;h1^=h3;if(has_blocks)h1^=acc;h1^=h1>>29;h1*=M1;h1^=h1>>31;h1*=M2;h1^=h1>>37;h1*=PHI;h1^=h1>>41;return h1;}

/* Reference: current safe with always-on accumulator */
uint64_t safe_old(const void*b,size_t l,uint64_t s){const uint8_t*p=b,*e=p+l;uint64_t ba=s^(l*PHI),h1=ba*PHI,h2=ba*M1,h3=ba*M2,h4=ba*M3,acc=ba^M2;
    while(p+32<=e){uint64_t v1=*(const uint64_t*)p;p+=8;uint64_t v2=*(const uint64_t*)p;p+=8;uint64_t v3=*(const uint64_t*)p;p+=8;uint64_t v4=*(const uint64_t*)p;p+=8;
        h1^=v1;h1=rot(h1+h2,11);h2^=v2;h2=rot(h2+h3,17);h3^=v3;h3=rot(h3+h4,23);h4^=v4;h4=rot(h4+h1,57);acc^=v1^v2^v3^v4;acc=rot(acc,31);acc*=PHI;}
    while(p+8<=e){uint64_t v=*(const uint64_t*)p;p+=8;h1^=v;h1=rot(h1+h2,11);acc^=v;acc=rot(acc,31);acc*=PHI;}
    uint64_t t=0;switch(e-p){case 7:t^=(uint64_t)p[6]<<48;case 6:t^=(uint64_t)p[5]<<40;case 5:t^=(uint64_t)p[4]<<32;case 4:t^=(uint64_t)p[3]<<24;case 3:t^=(uint64_t)p[2]<<16;case 2:t^=(uint64_t)p[1]<<8;case 1:t^=p[0];}if(t){h1^=t;h1=rot(h1+h2,11);acc^=t;acc=rot(acc,31);acc*=PHI;}
...
int main(void){
    printf("=== Tail-only skip accumulator ===\n\n");
    struct{const char*n;hf f;}hs[]={{"Fast",fast},{"OldSafe",safe_old},{"NewSafe",safe_new}};int nh=3;
