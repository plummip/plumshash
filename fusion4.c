...
static inline uint64_t mix128(uint64_t x){__uint128_t p=(__uint128_t)x*0x9E3779B97F4A7C15ULL;return(uint64_t)p^(uint64_t)(p>>64);}
...
/* Fusion4: 128-mul on h1 only (main data lane), ARX cross-mix for all */
uint64_t f4(const void*b,size_t l,uint64_t s){const uint8_t*p=b,*e=p+l;uint64_t ba=s^(l*PHI),h1=ba*PHI,h2=ba*M1,h3=ba*M2,h4=ba*M3;
    while(p+32<=e){
        h1=mix128(h1^*(const uint64_t*)p);p+=8;h2^=*(const uint64_t*)p;p+=8;h3^=*(const uint64_t*)p;p+=8;h4^=*(const uint64_t*)p;p+=8;
    ...
    h1^=h2;h3^=h4;h1^=h3;h1^=h1>>29;h1=mix128(h1);h1^=h1>>31;h1=mix128(h1);h1^=h1>>37;h1=mix128(h1);h1^=h1>>41;return h1;
}

/* Fusion5: 128-mul on all lanes, but ARX only every 2nd block (leaner Fusion1) */
uint64_t f5(const void*b,size_t l,uint64_t s){const uint8_t*p=b,*e=p+l;uint64_t ba=s^(l*PHI),h1=ba*PHI,h2=ba*M1,h3=ba*M2,h4=ba*M3;int blk=0;
    while(p+32<=e){
        ...
        if(blk&1){h1=rot(h1+h2,11);h2=rot(h2+h3,17);h3=rot(h3+h4,23);h4=rot(h4+h1,57);}
        blk++;
    }
    while(p+8<=e){h1=mix128(h1^*(const uint64_t*)p);p+=8;if(blk&1)h1=rot(h1+h2,11);blk++;}
    uint64_t t=0;switch(e-p){case 7:t^=(uint64_t)p[6]<<48;case 6:t^=(uint64_t)p[5]<<40;case 5:t^=(uint64_t)p[4]<<32;case 4:t^=(uint64_t)p[3]<<24;case 3:t^=(uint64_t)p[2]<<16;case 2:t^=(uint64_t)p[1]<<8;case 1:t^=p[0];h1=mix128(h1^t);if(blk&1)h1=rot(h1+h2,11);}
    h1^=h2;h3^=h4;h1^=h3;h1^=h1>>29;h1=mix128(h1);h1^=h1>>31;h1=mix128(h1);h1^=h1>>37;h1=mix128(h1);h1^=h1>>41;return h1;
}
...
int main(void){
    printf("=== Fusion 4/5 vs Vortex vs wyhash ===\n\n");
    struct{const char*n;hf f;}hs[]={{"wyhash64",wyhash64},{"Vortex",vortex},{"Fusion4",f4},{"Fusion5",f5}};int nh=4;
