...
int main(void){
    printf("═══════════════════════════════════════\n");
    printf("  PlumHash  vs  wyhash64\n");
    printf("═══════════════════════════════════════\n\n");
    struct{const char*n;hf f;}hs[]={{"PlumHash",plumhash},{"wyhash64",wyhash64}};int nh=2;
    uint8_t wb[1024];volatile uint64_t ws=0;
    for(int i=0;i<2000;i++){ws^=plumhash(wb,i%256?256:1024,i);ws^=wyhash64(wb,i%256?256:1024,i);}
    ...
    printf("\n─── AVALANCHE ───\n");
    for(int hi=0;hi<nh;hi++){uint8_t buf[256];for(int i=0;i<256;i++)buf[i]=i*0x9D+0x37;uint64_t base=hs[hi].f(buf,256,0);double lo=100,sum=0;
    ...
    printf("\n─── 64-bit COLLISIONS ───\n");
    for(int hi=0;hi<nh;hi++){uint64_t*ha=malloc(50000*8);for(int i=0;i<50000;i++)ha[i]=hs[hi].f(&i,4,0);
        uint32_t*s32=calloc(1<<20,4);int d=0;for(int i=0;i<50000;i++){uint32_t sl=ha[i]>>32&((1u<<20)-1),tg=ha[i];
