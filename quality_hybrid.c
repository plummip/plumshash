/*
 * quality_hybrid.c — SMHasher-grade quality checks for plumshash_hybrid v2
 * Mirrors smhasher_plums.c checks/thresholds (audit report section 1).
 * Compile: clang -O2 -o quality_hybrid quality_hybrid.c -lm
 */
...
static void test_sanity(void) {
    printf("-- Sanity --\n");
    TEST("deterministic", plumshash_hybrid("hello", 5, 42) == plumshash_hybrid("hello", 5, 42));
    TEST("seed_diff", plumshash_hybrid("hello", 5, 0) != plumshash_hybrid("hello", 5, 1));
    TEST("key_diff", plumshash_hybrid("hello", 5, 0) != plumshash_hybrid("world", 5, 0));
    TEST("empty_seed0_nonzero", plumshash_hybrid("", 0, 0) != 0);
    TEST("empty_key", plumshash_hybrid("", 0, 42) != 0);
    TEST("null_zero_len", plumshash_hybrid(NULL, 0, 42) != 0);
    uint8_t a[4] = {1,2,3,0}, b[3] = {1,2,3};
    TEST("len_matters", plumshash_hybrid(a, 4, 0) != plumshash_hybrid(b, 3, 0));
}
...
static void avalanche_at(size_t len, const char *label, double threshold) {
    ...
    for (int by = 0; by < nbytes; by++) {
        for (int bi = 0; bi < 8; bi++) {
            ...
            uint64_t h0 = plumshash_hybrid(buf, len, 42);
            buf[by] ^= (uint8_t)(1u << bi);
            uint64_t h1 = plumshash_hybrid(buf, len, 42);
...
static void test_differential(void) {
    ...
    for (int i = 0; i < n; i++)
        sum += popcount(plumshash_hybrid(buf, 64, i) ^ plumshash_hybrid(buf, 64, i + 1));
    double avg = sum / n;
    ...
    for (int i = 0; i < n; i++) {
        ...
        uint64_t h0 = plumshash_hybrid(buf, 64, 12345);
        memset(buf, (i + 1) & 0xFF, 64);
        uint64_t h1 = plumshash_hybrid(buf, 64, 12345 + i);
        sum += popcount(h0 ^ h1);
    }
    avg = sum / n;
...
static void test_chi2(void) {
    ...
    for (int i = 0; i < N; i++) {
        uint64_t r = rng();
        memcpy(key, &r, 8);
        bins[plumshash_hybrid(key, 8, i) & 0xFF]++;
    }
    double chi = 0.0;
    for (int i = 0; i < 256; i++) {
...
static void test_sparse(void) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        bins[(plumshash_hybrid(key, 64, 0) >> 48) & 0xFFFF]++;
    }
...
static void test_permutation(void) {
    ...
    uint64_t h0 = plumshash_hybrid(key, 16, 0);
    int diff = 0;
    for (int p = 1; p < 32; p++) {
        uint8_t t = key[0];
        memmove(key, key + 1, 15);
        key[15] = t;
        if (plumshash_hybrid(key, 16, 0) != h0) diff++;
    }
...
static void test_appended_zeroes(void) {
    ...
    for (int i = 0; i < 16; i++) {
        ...
        uint64_t a = plumshash_hybrid(key, i + 1, 0);
        key[i + 1] = 0;
        uint64_t b = plumshash_hybrid(key, i + 2, 0);
        if (a != b) diff++;
    }
...
static void test_seed_independence(void) {
    ...
    double sum = 0.0;
    for (int s = 0; s < 50; s++) {
        uint64_t a = plumshash_hybrid(key, 32, s);
        uint64_t b = plumshash_hybrid(key, 32, s + 1);
        sum += popcount(a ^ b);
    }
    double avg = sum / 50.0;
