/*
 * bench_compete.c — plumshash_hybrid vs the field: wyhash, XXH3, t1ha2,
 * rapidhash, plumshash. Two harnesses:
 ...
 * Compile: clang -O3 -march=native -o bench_compete bench_compete.c t1ha/t1ha2.c -I t1ha
 */
...
static hashfn variants[6];
static const char *vnames[6] = { "plums", "hybrid", "rapid", "wyhash", "xxh3", "t1ha2" };
...
int main(void) {
    variants[0] = plumshash; variants[1] = plumshash_hybrid; variants[2] = rapidhash_withSeed;
    variants[3] = wy_wrap; variants[4] = xx3_wrap; variants[5] = t1_wrap;
    ...
    for (int h = 0; h < NHARN; h++) {
        ...
        struct pair { int si, v; } pairs[31 * 6 * 3];
        int np = 0;
        for (int r = 0; r < 3; r++)
            for (int si = 0; si < nsizes; si++)
                for (int v = 0; v < 6; v++) { pairs[np].si = si; pairs[np].v = v; np++; }
        for (int i = np - 1; i > 0; i--) {
            int j = (int)(rng() % (uint64_t)(i + 1));
            struct pair t = pairs[i]; pairs[i] = pairs[j]; pairs[j] = t;
        }
        double best[31][6];
        for (int si = 0; si < nsizes; si++) for (int v = 0; v < 6; v++) best[si][v] = 1e30;
        double nsbest[31][6];
        ...
        printf("%-6s |", "Size");
        for (int v = 0; v < 6; v++) printf(" %16s", vnames[v]);
        printf("\n------ |");
        for (int v = 0; v < 6; v++) printf(" ----------------");
        printf("\n");
        for (int si = 0; si < nsizes; si++) {
