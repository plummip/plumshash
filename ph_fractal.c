...
/* Vortex baseline */
uint64_t vortex(const void*b,size_t l,uint64_t s){const uint8_t*p=b,*e=p+l;uint64_t ba=s^(l*VP),h1=ba*VP,h2=ba*VM1,h3=ba*VM2,h4=ba*VM3;while(p+32<=e){h1^=*(const uint64_t*)p;h1=rot(h1+h2,11);p+=8;h2^=*(const uint64_t*)p;h2=rot(h2+h3,17);p+=8;h3^=*(const uint64_t*)p;h3=rot(h3+h4,23);p+=8;h4^=*(const uint64_t*)p;h4=rot(h4+h1,57);p+=8;}while(p+8<=e){h1^=*(const uint64_t*)p;h1=rot(h1+h2,11);p+=8;}uint64_t t=0;switch(e-p){case 7:t^=(uint64_t)p[6]<<48;case 6:t^=(uint64_t)p[5]<<40;case 5:t^=(uint64_t)p[4]<<32;case 4:t^=(uint64_t)p[3]<<24;case 3:t^=(uint64_t)p[2]<<16;case 2:t^=(uint64_t)p[1]<<8;case 1:t^=p[0];h1^=t;h1=rot(h1+h2,11);}h1^=h2;h3^=h4;h1^=h3;h1^=h1>>29;h1*=VM1;h1^=h1>>31;h1*=VM2;h1^=h1>>37;h1*=VP;h1^=h1>>41;return h1;}

/* Fractal: 64-bit ARX + 32-bit self-similar pass on same data */
uint64_t fractal(const void*b,size_t l,uint64_t s){const uint8_t*p=b,*e=p+l;uint64_t ba=s^(l*VP),h1=ba*VP,h2=ba*VM1,h3=ba*VM2,h4=ba*VM3;while(p+32<=e){
    uint64_t d1=*(const uint64_t*)p;p+=8;uint64_t d2=*(const uint64_t*)p;p+=8;
    uint64_t d3=*(const uint64_t*)p;p+=8;uint64_t d4=*(const uint64_t*)p;p+=8;
    /* Level 1 (64-bit): standard ARX */
    h1^=d1;h1=rot(h1+h2,11);h2^=d2;h2=rot(h2+h3,17);h3^=d3;h3=rot(h3+h4,23);h4^=d4;h4=rot(h4+h1,57);
    /* Level 2 (32-bit): same pattern on halves — fractal self-similarity */
    uint32_t *h32=(uint32_t*)&h1;
    for(int i=0;i<4;i++){uint32_t a=h32[i],b=h32[(i+1)&3];a^=(uint32_t)(d1>>(i*16));a+=b;a=(a<<11)|(a>>21);h32[i]=a;}
}while(p+8<=e){h1^=*(const uint64_t*)p;h1=rot(h1+h2,11);p+=8;}uint64_t t=0;switch(e-p){case 7:t^=(uint64_t)p[6]<<48;case 6:t^=(uint64_t)p[5]<<40;case 5:t^=(uint64_t)p[4]<<32;case 4:t^=(uint64_t)p[3]<<24;case 3:t^=(uint64_t)p[2]<<16;case 2:t^=(uint64_t)p[1]<<8;case 1:t^=p[0];h1^=t;h1=rot(h1+h2,11);}h1^=h2;h3^=h4;h1^=h3;h1^=h1>>29;h1*=VM1;h1^=h1>>31;h1*=VM2;h1^=h1>>37;h1*=VP;h1^=h1>>41;return h1;}

int main(void){printf("=== Fractal Self-Similar Mixing ===\n\n");
    uint8_t wb[256];volatile uint64_t ws=0;for(int i=0;i<500;i++){ws^=vortex(wb,256,i);ws^=fractal(wb,256,i);}
    struct{const char*n;hf f;}hs[]={{"Vortex",vortex},{"Fractal",fractal}};int nh=2;
    printf("Speed (GB/s):\n%8s","");size_t szs[]={64,256,1024,65536};for(int si=0;si<4;si++)printf(" %8zu",szs[si]);printf("\n");
