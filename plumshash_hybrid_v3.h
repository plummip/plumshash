/*
 * plumshash_hybrid.h — plumshash v3 hybrid: 7-region, empirically selected
 * =========================================================================
 ...
 *   len <= 128  xxh3-style mid      independent mix16B pairs (ILP), wins
 *                                   20-45% latency over serial mum chains
 *   len <= 224  xxh3-style large    8 + walk mix16B, secret walk
 *   len <= 288  mum 7-lane          rapidhash >112 structure (112B/224B
 *                                   unrolled) — wins the 225-288B window
 ...
 *
 * Provenance: the <=224B paths mirror the structure of xxh3's short-key
 * functions (BSD-2-Clause, Yann Collet) with plumshash-family constants;
 * the 225-256B path mirrors rapidhash's >112 mum structure (MIT, Nicoshev);
 * the >256B path is the plumshash fast path (MPL-2.0). All constants are
 * plumshash-family. Note the xxh3-derived paths share xxh3's documented
 * seed-dependent multicollision property (17-240B, multiplication by zero).
 *
...
static PH_INLINE uint64_t ph_sec_qword(int byte_off) {
    uint64_t v; memcpy(&v, (const uint8_t*)ph_sec + byte_off, 8); return v;
}
...
/* ── 129-240 bytes: 8 mix16B + walking secret + last block ── */
static __attribute__((noinline)) uint64_t ph_large(const uint8_t * PH_RESTRICT p,
                                                   size_t len, uint64_t seed) {
    uint64_t acc = (uint64_t)len * PH_PHI;
    int nb = (int)len / 16;
    int i;
    for (i = 0; i < 8; i++)
        acc += ph_mix16B(p + 16 * i, ph_sec[2 * i], ph_sec[2 * i + 1], seed);
    acc = ph_aval(acc);
#pragma clang loop vectorize(disable)
    for (; i < nb; i++)
        acc += ph_mix16B(p + 16 * i, ph_sec_qword(16 * (i - 8) + 3),
                         ph_sec_qword(16 * (i - 8) + 11), seed);
    acc += ph_mix16B(p + len - 16, ph_sec_qword(119), ph_sec_qword(127), seed);
    return ph_aval(acc);
}

/* ── Mum path (rapidhash structure, plumshash constants): used for the
 * 4-8B tiny case (beats xxh3's rrmxmx there) and the 225-256B window
 * (7-lane 112B/224B unrolled blocks; wins that window on this device).
...
uint64_t plumshash_hybrid(const void *buf, size_t len, uint64_t seed) {
    ...
    if (len <= 128)            return ph_mid(p, len, seed);
    if (len <= 224)            return ph_large(p, len, seed);
    if (len <= 288)            return ph_mum_path(p, len, seed);   /* 225-288 */
